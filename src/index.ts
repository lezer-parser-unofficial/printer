...
export function printTree(
  cursor: TreeCursor | Tree | SyntaxNode,
  input: Input | string,
  { from, to, start = 0, includeParents, doColorize = false }: PrintTreeOptions = {}
): string {
  ...
  const colorize = (value: any, color: number) => {
    return doColorize ? "\u001b[" + color + "m" + String(value) + "\u001b[39m" : String(value)
  }
